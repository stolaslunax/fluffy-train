...
local function storeSingleFruit(fruit)
    ...
    if panelWas2 then guiPanel.Visible = false; task.wait(0.1) end

    local ap = collectBtn.AbsolutePosition
    local as = collectBtn.AbsoluteSize
    local touchX = ap.X + as.X / 2
    local touchY = ap.Y + as.Y / 2
    if not eb.IgnoreGuiInset then touchY = touchY - gi.Y end
    log("Store: tap Collect at " .. math.floor(touchX) .. "," .. math.floor(touchY) .. " IGI=" .. tostring(eb.IgnoreGuiInset))
    ...
    if getStoreCount() <= countBefore then
        log("Store: retry tap at offset=0")
        local ty2 = ap.Y + as.Y / 2
        pcall(function()
            VIM_S:SendTouchEvent("2", 0, touchX, ty2, game)
            task.wait(0.05)
            VIM_S:SendTouchEvent("2", 2, touchX, ty2, game)
        end)
    end

    if panelWas2 and guiPanel then task.wait(0.3); guiPanel.Visible = true end
...
mkSpacer(4, 6)
mkHdr(4, "MANUAL")
local btnEquip = mkBtn(4, "Equip Sword", Color3.fromRGB(50, 40, 80))
mkSpacer(4, 6)
...
btnEquip.MouseButton1Click:Connect(function() task.spawn(equipSword) end)
